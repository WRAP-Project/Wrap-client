...
**작업 브랜치를 따로 만들지 않습니다. 항상 `main`에서 직접 커밋하고
`main`으로 푸시합니다.** 기본 브랜치라는 이유로 브랜치를 새로 파거나
PR을 만들지 않습니다 — 프론트엔드 변경을 따로 코드 리뷰하는 사람이
없는 구조라 PR 단계가 의미가 없고, CI가 그 자리를 대신합니다.

- 커밋·푸시는 사용자가 요청할 때만 합니다.
- 커밋 전에 `npm run typecheck && npm run lint && npm run build`를 실행합니다.
- 변경이 여러 주제에 걸쳐 있으면 주제별로 나눠서 커밋합니다.
